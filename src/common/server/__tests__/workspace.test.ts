 ...
 * on one workspace. Run with: tsx --test src/common/server/__tests__/workspace.test.ts
 */
...
const CONFIG = path.join(REPO, "src", "common", "server", "config.ts");
...
/** Load src/common/server/config in a fresh process with the given cwd and env. */
