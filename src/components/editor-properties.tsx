...
import { z } from "zod";
import { Clip, Edl, Transition } from "@/lib/edl";
...
export function EditorProperties({ clip, edl, dispatch, onApplied, validationEdl, joint = null, mapOperations = ops => ops }: { clip: Clip; edl: Edl; dispatch: (ops: EditorOperation[]) => void; onApplied: () => void; validationEdl?: Edl; joint?: PropertyJoint | null; mapOperations?: (ops: EditorOperation[]) => EditorOperation[] }) {
  ...
  const [base] = useState({ clip, output: edl.output, transition: joint?.current ?? null });
  ...
  return <div className="space-y-4">
    ...
    <Button disabled={stale} onClick={() => {
      try {
        ...
        const mapped = [...mapOperations(operations), ...joined]; applyOperations(validationEdl ?? edl, mapped); dispatch(mapped); setError(""); onApplied();
