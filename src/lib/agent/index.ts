 ...
 * lib/config). Both of those resolve the workspace path at module load, so
 ...
 * Import `@/lib/agent/selection`, `@/lib/agent/detect` and
 * `@/lib/agent/models/cache` directly from the places that own a request.
 */
...
export { CURATED, curatedCatalog, prettyModelLabel, filterModels, mergeCatalog, orderModels } from "./models/catalog";
export type { ModelInfo, HarnessCatalog } from "./models/catalog";
