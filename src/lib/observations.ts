...
import type { Edl, Edit } from "./edl";
import type { EditorOperation } from "./editor/operations";
import { isGeneratedAuthor, describeAuthor } from "./editor/authorship";
...
export async function reviewObservations(projectId: string | undefined, o: { runner?: AgentProvider; provider?: string; model?: string; onEvent?: (e: AgentEvent) => void } = {}): Promise<Proposals> {
  ...
  const [{ listRules, ruleSchema }, { readGlossary }, { readPreferences }, { listTemplates }] = await Promise.all([import("./rules/registry"), import("./glossary"), import("./preferences"), import("./templates/registry")]);
