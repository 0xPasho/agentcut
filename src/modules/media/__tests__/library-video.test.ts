...
let mediaService: typeof import("../server/media-import");
...
before(async () => {
  ...
  [store, mediaService, tools, database, assets, registry] = await Promise.all([
    import("../../editor/server/store"), import("../server/media-import"), import("../../editor/server/tools"), import("../../../common/server/db"), import("../server/assets"), import("../../templates/server/registry"),
  ]);
...
test("an end card arrives at the loudness of the video it is stuck on, unless it is told not to", async () => {
  ...
  const { loudness } = await import("../server/ffmpeg");
