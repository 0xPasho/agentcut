...
import { Clip, DEFAULT_ITEM_TRANSFORM, Edl, type Edit } from "../types";
import { snapAxis, snapDraggedSpan, snapSpan, snapTargets, snapTime } from "../lib/snapping";
import { classifyFile, dropDuration, parseDrag } from "../lib/dnd";
import { applyOperations } from "../lib/operations";
import { invertOperations } from "../lib/history";
import { buildTimelineGroupMove, buildTimelineSlip, timelineCollides } from "../lib/timeline-interactions";
...
test("titles, pictures and captions move inside their frame in the frame's own units", async () => {
  const { moveOverlay, overlayLabel } = await import("../lib/canvas");
