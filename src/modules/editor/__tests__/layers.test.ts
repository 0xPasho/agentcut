...
import { Edl, Clip, DEFAULT_ITEM_TRANSFORM } from "../types";
import { applyOperations, type EditorSnapshot } from "../lib/operations";
...
before(async () => {
  ...
  database = await import("../../../common/server/db"); store = await import("../server/store"); tools = await import("../server/tools");
});
...
test("transitions undo through the same operations, and carry who placed them", async () => {
  ...
  const { stampAuthor } = await import("../lib/authorship");
  const { describeAuthor } = await import("../lib/authorship");
  const edl = fixture();
...
test("an equal-power crossfade holds its loudness through the joint", async () => {
  const { crossfadeGain } = await import("../lib/sequences");
...
test("a layer with no keyframes is the static transform it always was, and one keyframe holds", async () => {
  const { animatedAt } = await import("../lib/keyframes");
...
test("every named ease is a different way of getting there, and all of them arrive", async () => {
  const { animatedAt, EASES } = await import("../lib/keyframes");
...
test("the way out of that refusal is an ordinary keyframe list, not a way around the operation", async () => {
  const { clearField, setKeyframe } = await import("../lib/motion");
  const { animatedAt, animatedFields } = await import("../lib/keyframes");
...
test("motion undoes through the same operations, refuses a stale form, and carries who placed it", async () => {
  const { invertOperations } = await import("../lib/history");
  const { stampAuthor, describeAuthor } = await import("../lib/authorship");
  const edl = fixture();
...
test("the panel's shortcuts are ordinary keyframe lists, Ken Burns included", async () => {
  const { kenBurns, pinPlacement, pinVolume, setKeyframe, retimeKeyframe, removeKeyframe } = await import("../lib/motion");
  const { animatedAt, itemSeconds } = await import("../lib/keyframes");
...
test("a shot the trim left keyframes hanging off is still an editable shot", async () => {
  const { setKeyframe } = await import("../lib/motion");
...
test("a retime that lands on the keyframe at the end steps back rather than off the shot", async () => {
  const { retimeKeyframe, setKeyframe } = await import("../lib/motion");
