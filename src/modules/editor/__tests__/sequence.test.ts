...
before(async () => {
  ...
  [store, mediaService, tools, database] = await Promise.all([import("../server/store"), import("../../media/server/media-import"), import("../server/tools"), import("../../../common/server/db")]);
...
test("folder tools browse supported media without recursion and import reusable project assets", async () => {
  ...
  try {
    ...
    const first = await tools.executeEditorTool(a.id, { tool: "assets.browseLocal", folder: originals }) as import("../../media/server/local-assets").FolderListing;
    ...
    const importedA = await tools.executeEditorTool(a.id, { tool: "assets.importLocal", file: original }) as import("../../../common/server/db").AssetRow;
...
test("the assemble endpoint accepts a name alone, a file list, or neither validly", async () => {
  const { POST } = await import("../../../app/api/projects/assemble/route");
  const { NextRequest } = await import("next/server");
...
test("canvas segments are ordinary shots without a source, through HTTP and tools alike", async () => {
  ...
  const { PATCH } = await import("../../../app/api/projects/[id]/route"); const { NextRequest } = await import("next/server");
  ...
  assert.equal(response.status, 200);
  assert.deepEqual((await response.json()).edl, expected);
  const { assetEdit } = await import("../lib/asset-edit");
