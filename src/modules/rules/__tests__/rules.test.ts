...
test("the selection agent's clips carry tags and judged rules, and the host executes them after publishing", async () => {
  const { buildEdl, readRuleMatches } = await import("../../clipping/server/select");
  const { applyMatchedRules } = await import("../../project/server/jobs");
...
test("preferences are the owner's words, kept per level and handed to every agent as one block", async () => {
  ...
  const read = await tools.executeEditorTool(id, { tool: "preferences.get" }) as import("../server/preferences").Preferences;
