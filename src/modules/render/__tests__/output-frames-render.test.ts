 ...
 * lives in src/modules/agent/__tests__/conversation.test.ts.
 *
...
test("the agent's frames carry the title the footage never had", { timeout: 240_000 }, async () => {
  ...
  const { grabFrame } = await import("../../media/server/ffmpeg");
...
test("a turn that changed nothing renders nothing; one that changed the picture renders again", { timeout: 240_000 }, async () => {
  const { outputFrames } = await import("../server/frames");
  const { readEditor, editProject } = await import("../../editor/server/store");
