...
import { recordMessage, sendMessage } from "./conversation";
import { formatActivity } from "../../project/lib/activity";
import { recordActivity, runReportedTool } from "../../project/server/activity-log";
...
const DESCRIPTIONS: Record<string, string> = {
  ...
  "packs.style.get": "A pack's style guide (STYLE.md) and its reference videos, each with the still an agent can open.",
  "packs.style.set": "Replace a pack's style guide. Refused if it is over 4000 characters or reads as instructions to the agent.",
  "packs.examples.add": "Copy a local video or picture into a pack as a reference, with a title and a note on what to take from it.",
  "packs.examples.update": "Change a reference's title or note.",
  "packs.examples.remove": "Remove a reference from a pack.",
  "packs.review.get": "What a pack holds its videos to: its measured checks and its questions, with anything wrong with them.",
  "packs.review.set": "Replace a pack's standard. Refused if a question or a fix sentence reads as instructions to the agent; a check nobody can measure is saved with a warning.",
  "style.active": "Which style guide this project's videos are made to, and why.",
  "style.choose": "Set this project's style guide by pack id, \"none\" for none, or null to decide on its own.",
  "review.run": "Hold a video to the standard its pack sets and write down what came of it: the measured checks against the project and, once it is exported, against its own pixels. rubric:true also has an agent answer the pack's questions with evidence, which costs a run. This is the difference between \"the template was applied\" and \"this is a video the channel would publish\".",
  "review.read": "The last review of a video, or of every video in the project.",
  "review.criteria": "What this video is held to and where it comes from: the pack's checks and questions over the built-in ones.",
  "review.catalogue": "Every metric a pack may hold a video to. This is the vocabulary review.json is written in; a check naming anything else never runs.",
  "review.waive": "Let one finding stand, with the reason. The review still reports it, with the reason beside it.",
  "review.unwaive": "Take a waiver back.",
  "review.severity": "What a finding is worth here: critical, suggestion or nitpick; null gives it back the pack's. Lowering one takes a reason.",
