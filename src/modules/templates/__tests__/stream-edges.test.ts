...
test("a video recorded quietly is placed at the loudness the template asks for, and the card follows it there", async () => {
  ...
  const { loudness } = await import("../../media/server/ffmpeg");
...
test("a single peak in quiet footage is not a reason to make the whole video quieter", async () => {
  ...
  const { loudness, audioLevel } = await import("../../media/server/ffmpeg");
  ...
  const { targetLevel } = await import("../server/apply");
  const template = (await import("../lib/resolve")).resolveTemplate(
...
test("how wide a line is, decided without measuring it", async () => {
  const { emWidth, fitScale, fitScaleAll, MIN_FIT } = await import("../../../common/lib/text-fit");
...
test("room noise the transcript talks over is reported, not quietly cut", async () => {
  const { deadAir, MIN_DEAD_SEC } = await import("../lib/quiet");
  const step = 0.5;
...
test("a dry run hears the hole in the footage before anything is rendered", async () => {
  ...
  const { buildTimeMap } = await import("../../editor/lib/timeline");
  const map = buildTimeMap(clip);
...
test("a rule that would do nothing is refused where it is written", async () => {
  const registryRules = await import("../../rules/server/registry");
...
test("four hundred transcripts of caption lines, and none of them loses a word or shows a stranger", async () => {
  ...
  const { toLines, lineAt, activeWordIndex, visibleWords, LINE_LEAD, LINE_TAIL } = await import("../../editor/lib/timeline");
