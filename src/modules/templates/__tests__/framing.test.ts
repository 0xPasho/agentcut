...
test("an aspect variant can reframe: the same template, a different shape, its own split", async () => {
  ...
  const { resolveTemplate } = await import("../lib/resolve");
...
test("a template replaces the draft the clip selection made, and leaves a hand edit alone", async () => {
  ...
  const { describeAuthor } = await import("../../editor/lib/authorship");
...
test("the pace of a finished video is measurable, and the settings that reproduce it are findable", async () => {
  const pace = await import("../lib/pace");
...
test("a rectangle that loses a third of itself to the shape of its half says so", async () => {
  ...
  const resolved = (await import("../lib/resolve")).resolveTemplate(streamShort, { aspect: "9:16" });
...
test("the style audit is a tool, and says so when there is nothing exported to read", async () => {
  ...
  await tools.executeEditorTool(id, { tool: "template.apply", templateId: "stream-split", sequenceId, expectedRevision: store.readEditor(id).revision });
  const audits = await tools.executeEditorTool(id, { tool: "style.audit" }) as import("../../render/server/style-check").StyleAudit[];
  ...
  const one = await tools.executeEditorTool(id, { tool: "style.audit", sequenceId }) as import("../../render/server/style-check").StyleAudit[];
