...
import { resolveTemplateComment } from "../../stream-comments/server/resolve";
import { aspectOf, resolveTemplate } from "../lib/resolve";
import type { VideoTemplate } from "../types";
import type { Heard, ImageCue, Subject } from "../lib/script";
import type { Envelope } from "../lib/quiet";
import {
...
async function resolveCue(context: Context, item: SequenceItem, cue: ImageCue): Promise<CueResolution> {
  ...
  for (const source of template.images.sources) {
    ...
    if (kind === "frame") {
      ...
      const { captureFrameAsset } = await import("../../editor/server/tools");
...
async function resolveBookend(
  ...
  const { toAbs } = await import("../../media/server/assets");
  const { probe, loudness } = await import("../../media/server/ffmpeg");
...
async function envelopesFor(edl: Edl, sequenceId: string): Promise<Map<string, Heard>> {
  ...
  const { loudnessCurve } = await import("../../media/server/ffmpeg");
