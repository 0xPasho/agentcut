...
let mcp: typeof import("../../agent/server/mcp");
let activity: typeof import("../lib/activity");
let activityLog: typeof import("../server/activity-log");
...
before(async () => {
  ...
  [store, database, tools, conversation, mcp, activity, activityLog] = await Promise.all([
    import("../../editor/server/store"), import("../../../common/server/db"), import("../../editor/server/tools"),
    import("../../agent/server/conversation"), import("../../agent/server/mcp"), import("../lib/activity"), import("../server/activity-log"),
  ]);
});
...
test("the thread reads as one conversation: what was asked, the steps that answered it, the reply", async () => {
  const { buildThread } = await import("../../agent/lib/thread");
...
test("the chat is an entry point: a message, a dropped video and a link each start a project their own way", async () => {
  const { readStart, nameFromMessage } = await import("../../agent/hooks/use-chat");
