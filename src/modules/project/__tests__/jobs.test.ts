...
let reaper: typeof import("../server/reaper");
...
before(async () => {
  ...
  [database, reaper, jobs] = await Promise.all([
    import("../../../common/server/db"), import("../server/reaper"), import("../server/jobs"),
  ]);
});
...
function insertJob(projectId: string, over: Partial<import("../../../common/server/db").JobRow> = {}) {
  const now = Date.now();
  const job = {
  ...
  } as import("../../../common/server/db").JobRow;
...
test("the agent has the same escape hatch as the panel", async () => {
  ...
  const job = insertJob(id, { pid: process.pid, boot_id: reaper.BOOT_ID });
  reaper.claim(job.id, id);
  const { executeEditorTool, projectStatus } = await import("../../editor/server/tools");
...
test("a status poll reaps a job whose process is gone", async () => {
  const id = project("agent");
  insertJob(id);
  const { projectStatus } = await import("../../editor/server/tools");
  assert.equal(projectStatus(id).working, false);
...
test("a render lock left by a dead process is taken over, not obeyed forever", async () => {
  const id = project();
  const dir = (await import("../../../common/server/config")).projectDir(id);
  const lockPath = path.join(dir, "render.lock");
  await fs.writeFile(lockPath, JSON.stringify({ pid: DEAD_PID, at: Date.now() }));
  const render = await import("../../render/server/render-project");
 ...
 */
const background = (projectId: string, over: Partial<import("../../../common/server/db").JobRow> = {}) =>
