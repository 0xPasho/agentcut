...
let batch: typeof import("../src/lib/batch");
...
before(async () => {
  ...
  [store, mediaService, database, batch, conversation] = await Promise.all([
    import("../src/lib/editor/store"), import("../src/lib/editor/media"), import("../src/lib/db"), import("../src/lib/batch"), import("../src/lib/editor/conversation"),
  ]);
...
test("transcribing imported media puts words on its shots and a missing recogniser is reported, not thrown", async () => {
  ...
  const { transcribeProjectMedia } = await import("../src/lib/transcribe/media");
